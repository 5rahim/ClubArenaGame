...
var Model = /** @class */ (function () {
    ...
    Model.prototype.all = function () {
        ...
        return new Promise(function (resolve, reject) {
            DataAccess_1.default.connection.query('SELECT * FROM ' + _this.table + '', function (err, rows) {
                if (err)
                    reject(err);
                if (rows)
                    resolve(rows[0]);
    ...
    Model.prototype.updateBy = function (by, byValue, key, value) {
        DataAccess_1.default.connection.query('UPDATE ' + this.table + ' SET ' + key + ' = ? WHERE ' + by + ' = ?', [value, byValue], function (err, rows) {
            if (err)
                console.log(err);
        });
    };
